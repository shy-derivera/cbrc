	...
	<div class="wrapper">
		...
		<div class="page-wrapper">
			<div class="page-content">
	            ...
	            $lec_id=$_GET['lec_id'];
				$program=$_GET['rev_prog'];
	            $query=mysqli_query($conn,"select * from `es_schedule` where lecturer_id ='$lec_id' AND review_program='$program'");
	            $row=mysqli_fetch_array($query);
            ?>
										<form method="POST" action="process_update_sched.php?lec_id=<?php echo $lec_id; ?>" enctype="multipart/form-data" class="row g-3">
											<input type="hidden" name="lec_id" value="<?php echo $_REQUEST['lec_id']; ?>">
											<input type="hidden" name="sched_id" value="<?php echo $_REQUEST['sched_id']; ?>">
											...
											<select id ="group" class="form-control" name="review_program" required>
								             <option value="<?php echo $row['review_program']; ?>"><?php echo $row['review_program']; ?></option>
								<?php
								    include('connection.php');
								...
								?>
									<option value="<?php echo $f_gtransaction['program_description']?>"><?php echo $f_gtransaction['program_description'] . ' (' . $f_gtransaction['program_fee'] . ')'?></option>
								<?php
										}
									$conn->close();	
								?>
											...
											<div class="col-sm-2">
											 ...
											 <input type="time" name="from" value="<?php echo $row['time_from']; ?>" class="form-control" >
									        </div>
											<div class="col-sm-2">
											 <label for="">To</label>
											 <input type="time" name="to" value="<?php echo $row['time_to']; ?>" class="form-control" >
									        </div>
									        <div class="col-sm-4">
